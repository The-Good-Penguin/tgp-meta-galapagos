...
The CVE scanning class to enable depends on your Yocto LTS release:

**Any version prior to Wrynose:**
...
Note: `GALAPAGOS_EXTRA_PACKAGES` is not supported when using `sbom-cve-check`.
Instead, `sbom-cve-check` derives CVE coverage from the SPDX SBOM generated by
`create-spdx-3.0`, which covers packages installed into the rootfs (i.e.
`IMAGE_INSTALL`). Packages built but not installed into the rootfs, such as
bootloaders or firmware added via `EXTRA_IMAGEDEPENDS` or `WKS_FILE_DEPENDS`,
are not included. 
...
Galapagos can also produce a PDF license report from the license.manifest
generated by Yocto. Please update your local.conf to include the following:

    INHERIT += "galapagos-license-check"

The license report uses the same GALAPAGOS variables described above.
Please note that your product key must have the "license" feature
enabled, this can be requested by emailing <sales@thegoodpenguin.co.uk>.

If you wish for license reports to be sent to different recipients or at
a different interval to CVE reports, you can use the following optional
variables:

    GALAPAGOS_LICENSE_EMAIL="licenses@example.co.uk"
    GALAPAGOS_LICENSE_INTERVAL="weekly"
